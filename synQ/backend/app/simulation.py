...
def generate_recovery_plans(
    ...
    # ─── Strategy 5: Repair the broken machine (only for MACHINE_BREAKDOWN) ──
    # Unlike strategies 1-4 (which all exclude the broken machine entirely),
    # this strategy keeps it in play: an available qualified maintenance
    # worker repairs it over a configured duration, independent tasks keep
    # running on other machines in the meantime, and tasks that need the
    # broken machine (plus their dependents) wait for it and are rescheduled
    # after repair. This is the only strategy that can succeed when the
    # broken machine has no alternative (e.g. "quality_testing" with a single
    # tester machine) — in that case plans A-D are correctly infeasible, and
    # this one is the fallback recovery path.
    if d_type == DisruptionType.MACHINE_BREAKDOWN:
        plan5 = generate_repair_plan(db, disruption, machines, workers)
        db.add(plan5)
        plans.append(plan5)
...
# Skill string a worker needs to be considered qualified to perform a repair.
# Matches the same free-text-skill pattern already used for production
# skills (WorkerSkill.skill) — no new "is this a maintenance worker" flag is
# needed, a worker just needs this skill among their existing skills.
MAINTENANCE_SKILL = "maintenance"

# Only used if a machine has no configured repair_duration_minutes AND the
# disruption itself was created with no duration_hours parameter — a
# last-resort, clearly-documented default, not a fabricated per-scenario
# number.
DEFAULT_REPAIR_DURATION_MINUTES = 240  # 4 hours
...
def generate_repair_plan(
    ...
    """
    Repair-based recovery for a MACHINE_BREAKDOWN disruption:
    find a qualified, available maintenance worker -> determine the repair
    duration -> compute when the machine is available again -> generate a
    schedule where only tasks needing that machine (and their dependents)
    wait for it, while everything else proceeds -> validate the result with
    the same deterministic scheduler/validator used everywhere else.

    Returns a RecoveryPlan; on any failure to find a worker or produce a
    valid schedule, it comes back with is_feasible=False and a clear reason
    instead of fabricating a result.
    """
    factory_id = disruption.factory_id
    params = disruption.parameters or {}
    machine = next((m for m in machines if m.id == disruption.affected_resource_id), None)

    base_kwargs = dict(
        disruption_id=disruption.id,
        name="Plan E: Repair Machine",
        strategy="repair_machine",
        description="Repair the broken machine using a qualified maintenance worker. "
                    "Tasks that don't need it keep running; tasks that do wait and are "
                    "rescheduled once the machine is back online.",
    )

    if not machine:
        return RecoveryPlan(**base_kwargs, is_feasible=False,
                             infeasibility_reason="Affected machine not found.")

    # Find an available, qualified maintenance worker (existing Worker.available
    # flag + existing WorkerSkill relationship — no new worker-qualification
    # concept introduced).
    maintenance_worker = next(
        (w for w in workers if w.available and MAINTENANCE_SKILL in [s.skill for s in w.skills]),
        None,
    )
    if not maintenance_worker:
        return RecoveryPlan(**base_kwargs, is_feasible=False,
                             infeasibility_reason=(
                                 f"No available worker with the '{MAINTENANCE_SKILL}' skill was found — "
                                 "cannot repair the machine without a qualified technician."
                             ))

    # Repair duration: machine's own configured value first, then the
    # disruption's own duration_hours parameter (already used elsewhere for
    # this same disruption type), then a documented fallback constant.
    if machine.repair_duration_minutes:
        repair_duration_minutes = machine.repair_duration_minutes
        duration_source = "machine configured repair_duration_minutes"
    elif params.get("duration_hours"):
        repair_duration_minutes = float(params["duration_hours"]) * 60
        duration_source = "disruption duration_hours parameter"
    else:
        repair_duration_minutes = DEFAULT_REPAIR_DURATION_MINUTES
        duration_source = "default fallback (no configured value available)"

    # This recovery schedule (like every other strategy) is generated fresh
    # from a t=0 baseline, so repair begins at t=0 — consistent with how
    # analyze_impact and strategies 1-4 already treat "now".
    repair_start = 0.0
    machine_available_time = repair_start + repair_duration_minutes

    try:
        schedule, errors = generate_schedule(
            db, factory_id,
            name="Recovery: Repair Machine",
            excluded_machine_ids=set(),   # machine stays in play, just not ready yet
            excluded_worker_ids=set(),
            order_overrides={},
            strategy="default",
            commit=False,
            machine_available_from={machine.id: machine_available_time},
        )
    except Exception as e:
        return RecoveryPlan(**base_kwargs, is_feasible=False, infeasibility_reason=str(e))

    if not schedule:
        reason = "; ".join(e.get("error", "") for e in errors) if errors else "No feasible schedule could be generated."
        return RecoveryPlan(**base_kwargs, is_feasible=False, infeasibility_reason=reason)

    db.flush()
    is_valid, validation_errors = validate_schedule(db, schedule.id)
    is_feasible = (len(errors) == 0) and is_valid
    infeasibility_reason = "; ".join([e.get("error", "") for e in errors] + validation_errors) if not is_feasible else ""

    tasks = db.query(ScheduleTask).filter(ScheduleTask.schedule_id == schedule.id).all()
    order_map = {o.id: o for o in db.query(Order).filter(Order.factory_id == factory_id).all()}
    step_cache: Dict[int, ProductionStep] = {}

    def step_of(step_id: int) -> Optional[ProductionStep]:
        if step_id not in step_cache:
            step_cache[step_id] = db.query(ProductionStep).filter(ProductionStep.id == step_id).first()
        return step_cache[step_id]

    def task_ref(t: ScheduleTask) -> Dict[str, Any]:
        order = order_map.get(t.order_id)
        step = step_of(t.step_id)
        return {
            "task_id": t.id, "order_id": t.order_id,
            "order_code": order.order_code if order else "?",
            "step_id": t.step_id, "step_name": step.name if step else "?",
            "start_time": t.start_time, "end_time": t.end_time,
        }

    # Tasks directly on the broken machine (these necessarily wait for repair).
    waiting_tasks = [t for t in tasks if t.machine_id == machine.id]
    waiting_keys = {(t.order_id, t.step_id, t.unit_index) for t in waiting_tasks}

    # Their direct dependents (one hop, same convention analyze_impact already
    # uses for "downstream_tasks") — the rescheduled blocked task's immediate
    # dependents, not a full transitive closure.
    dependent_tasks = []
    for t in tasks:
        if t.machine_id == machine.id:
            continue
        ...
        if step and step.preceding_steps:
            for dep_id in step.preceding_steps:
                if (t.order_id, dep_id, t.unit_index) in waiting_keys:
                    dependent_tasks.append(t)
                    break

    # Everything else on OTHER machines that starts before the machine is
    # repaired: independent production that keeps running during the repair.
    continued_tasks = [
        t for t in tasks
        if t.machine_id != machine.id and t.start_time < machine_available_time
    ]

    final_completion_time = max((t.end_time for t in tasks), default=0.0)
    metrics = dict(schedule.metrics or {})
    metrics["repair"] = {
        "maintenance_worker": {"id": maintenance_worker.id, "name": maintenance_worker.name},
        "repair_duration_minutes": repair_duration_minutes,
        "repair_duration_source": duration_source,
        "repair_start": repair_start,
        "machine_available_time": machine_available_time,
        "waiting_tasks": [task_ref(t) for t in waiting_tasks],
        "dependent_tasks_rescheduled": [task_ref(t) for t in dependent_tasks],
        "tasks_continued_during_repair": [task_ref(t) for t in continued_tasks],
        "final_completion_time": final_completion_time,
        "delay_minutes": (schedule.metrics or {}).get("total_delay", 0.0),
        "feasible": is_feasible,
    }

    return RecoveryPlan(
        **base_kwargs,
        schedule_data=serialize_schedule_tasks(db, schedule),
        metrics=metrics,
